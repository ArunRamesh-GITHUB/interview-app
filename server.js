...
import { mountAffiliateRoutes } from './server/routes/affiliateRoutes.js'
...
// Mount Affiliate routes (after helper functions are defined below)
// ---------- Helpers ----------
...
// Mount Affiliate routes (now that helpers are defined)
mountAffiliateRoutes(app, { sbAdmin, currentUser, authRequired, requireServer })

// ---------- AUTH ----------
app.post('/api/register', lightLimiter, async (req, res) => {
  try {
    const { email, password, username, affiliate_slug } = req.body || {}
    ...
    // Attribute user to affiliate if slug provided (one-time, locked)
    if (affiliate_slug) {
      try {
        const { data: attrResult, error: attrError } = await sbAdmin.rpc('sp_attribute_user', {
          p_user_id: userId,
          p_affiliate_slug: affiliate_slug,
          p_source: 'web_signup'
        })
        if (attrError) {
          console.warn('Affiliate attribution failed:', attrError.message)
        } else if (attrResult?.attributed) {
          console.log(`✅ User ${userId} attributed to affiliate ${affiliate_slug}`)
        }
      } catch (attrErr) {
        console.warn('Affiliate attribution error:', attrErr.message)
      }
    }
...
app.get('/api/me', async (req, res) => {
  try {
    ...
    // Lookup user's affiliate attribution (for mobile app to know)
    let affiliate_slug = null
    try {
      const { data: attrData } = await sbAdmin
        .from('user_attribution')
        .select('affiliate_slug')
        .eq('user_id', u.id)
        .maybeSingle()
      affiliate_slug = attrData?.affiliate_slug || null
    } catch (attrErr) {
      console.warn('Attribution lookup error:', attrErr.message)
    }

    res.json({
      user: { id: u.id, email: u.email, affiliate_slug },
      usage: {
